...
CREATE OR REPLACE FUNCTION manually_update_daily_stats(stats_date date)
RETURNS void AS $$
BEGIN
  ...
  SELECT
    ...
    COUNT(DISTINCT o.id),
    COUNT(DISTINCT p.id) FILTER (WHERE DATE(p.created_at) = stats_date),
    COUNT(ae.id) FILTER (WHERE ae.event_type = 'page_view')
  FROM (SELECT stats_date AS ref_date) d
  LEFT JOIN orders o ON DATE(o.created_at) = stats_date
  LEFT JOIN profiles p ON DATE(p.created_at) = stats_date
  LEFT JOIN analytics_events ae ON DATE(ae.created_at) = stats_date
  ON CONFLICT (date) 
  DO UPDATE SET
